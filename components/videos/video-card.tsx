...
export function VideoCard({
  ...
  return (
    <article>
      <Card
        className={cn(
          "grid gap-0 py-0 transition-shadow hover:shadow-md sm:grid-cols-[7.5rem_minmax(0,1fr)]",
          className
        )}
      >
        <Link
          href={video.href}
          className="group relative aspect-video w-full shrink-0 overflow-hidden bg-muted sm:aspect-auto sm:h-full sm:min-h-32"
        ...
        >
          ...
          <span className="absolute inset-0 flex items-center justify-center">
            <span className="flex size-10 items-center justify-center rounded-full bg-background/90 text-primary shadow-sm backdrop-blur">

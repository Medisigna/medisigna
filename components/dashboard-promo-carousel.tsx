...
const promoItems = [
  {
    ...
    description:
      "Tanyakan aturan pakai dan efek samping obat langsung ke apoteker.",
  ...
  },
  {
    ...
    description:
      "Pastikan dosis, waktu minum, dan cara pakai obat lebih jelas.",
    imageSrc:
      "https://images.unsplash.com/photo-1610046869459-b4d6402043da?ixlib=rb-4.1.0&q=85&fm=jpg&crop=entropy&cs=srgb&w=1200",
  ...
  },
  {
    ...
    description:
      "Pilih apoteker yang sudah melewati proses verifikasi Medisigna.",
        ...
        <CarouselContent>
          {promoItems.map((item, index) => (
            <CarouselItem key={item.title}>
              {variant === "soft" ? (
                <div className="relative min-h-56 overflow-hidden rounded-[1.75rem] bg-muted md:min-h-72">
                  <img
                    src={item.imageSrc}
                    alt={item.imageAlt}
                ...
                <div className="relative flex min-h-40 items-end overflow-hidden rounded-2xl bg-primary text-primary-foreground shadow-[0_18px_40px_-28px_rgba(14,47,89,0.6)] ring-1 ring-primary/20 md:min-h-52">
                  ...
                  <div className="relative flex max-w-xl flex-col gap-3 p-5 md:p-7">
                    <p className="w-fit rounded-md bg-white/15 px-2 py-1 text-[11px] leading-none font-medium text-white backdrop-blur-sm">
                      Medisigna
                    </p>
